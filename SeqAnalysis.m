...
descriptorCgrLBP=0;
descriptorCgrHist=0;
descriptor1BaseLBP=0;
descriptor2BaseLBP=0;
descriptor3BaseLBP=0;
descriptorCgrHistLBP=0;
descriptorCgrBlockLBP=0;
descriptorCgrLBPTriangle=0;
descriptorCgrHist1=0;
descriptorGLCM=0;
descriptorSeqLBP256=0;
descriptorDWTCGRLBP=0;
descriptorDWTCGRFFT=0;
descriptorPCR=0;
descriptorPartWiseLBP=0;
descriptorSeqDiff=0;
...
if avgLength>=1 && avgLength<=99999
    k=8;
elseif avgLength>=100000 && avgLength<=9999999
    k=9;
else
    k=10;
end
...
%     
    ...
    CgrImage=CGRImageKmerCount(sequence,kmer);
    %CgrImage=imresize(CgrImageOrg,[64 64]);
    [row,col]=size(CgrImage);
    CgrOneDim= CgrImage(:);
    descriptorCgrHist1(seqIter,1:row*col)=CgrOneDim;
    
    reductionRate=4;%it may 4, 16, 64, and 256
    reducedMatrix=MatrixReduction(CgrImage,reductionRate);
    [row,col]=size(reducedMatrix);
    CgrReducedOneDim= reducedMatrix(:);
    descriptorCgrReduced(seqIter,1:row*col)=CgrReducedOneDim;
    
    disp(seqIter);
end


descriptor=descriptorCgrReduced;
